...
It is possible to modify multiple properties of a DID document. In order to do so, it is necessary to pass an array of the xref:components/update-did/api.adoc#type-did-update-operation[operations], which will contain the changes that are to be made. In this case, a new message will be created for each update, which will require a separate signature. The cost of performing such an operation will change in accordance with the number of changes and thus with the number of DID messages that need to be sent to Topic.
