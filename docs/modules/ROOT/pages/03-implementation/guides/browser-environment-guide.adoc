...
By default, Registrar and Resolver uses a xref::03-implementation/components/topic-reader-api.adoc[TopicReader] to read messages from the Hedera network topic. Default version of the TopicReader uses gRPC to connect to the Hedera network. This won't work in the browser environment.
