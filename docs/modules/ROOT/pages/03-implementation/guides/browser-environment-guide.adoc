...
[source,bash]
----
npm install @hiero-ledger/sdk @hiero-did-sdk/registrar @hiero-did-sdk/resolver
----
...
IMPORTANT: Make sure that your module bundler is configured to resolve `browser` field in `package.json` files. This is required to load the correct version of the DID SDK in the browser environment.
