...
|privateKey?
|`string` \| PrivateKey`
|A private key (in DER format) or a `PrivateKey` object used to generate a xref:03-implementation/components/signer-guide.adoc[Signer] for signing the DID update message.
...
The Hiero DID SDK provides a `updateDID` function within its `registrar` package. For further details, refer to the xref:04-deployment/packages/index.adoc#essential-packages[`@hiero-did-sdk-js/registrar`] package documentation.
