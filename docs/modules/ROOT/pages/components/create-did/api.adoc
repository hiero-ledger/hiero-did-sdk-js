...
* xref:components/create-did/guide.adoc[Creating a new DID Document]
...
|privateKey?
|`PrivateKey`
|A xref:architecture/private-key.adoc[Private Key] that was used to sign a DID message. Only available if `signer` or `privateKey` was not specified.
|=== 
