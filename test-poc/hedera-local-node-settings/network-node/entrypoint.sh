...
ls -la /opt/hgcapp/services-hedera/HapiApp2.0
cat /opt/hgcapp/services-hedera/HapiApp2.0/config.txt
/usr/bin/env java ${JAVA_HEAP_OPTS} ${JAVA_OPTS} -cp "${JAVA_CLASS_PATH}" "${JAVA_MAIN_CLASS}" -Dlog4j.configurationFile=/opt/hgcapp/services-hedera/HapiApp2.0/log4j2.xml --trace
EC="${?}"
