...
import { deactivateDID, DeactivateDIDResult, DIDUpdateOperation, updateDID, UpdateDIDResult } from '@swiss-digital-assets-institute/registrar';
...
When('the `deactivateDID` is called with the SDK Client', function (this: DIDWorld, done) {
...
Then('the function should return deactivated DID Document', async function (this: DIDWorld) {
    const did: string = this.sharedData['deactivatedDid']
    const signer = new Signer(this.sharedData['operatorPrivateKey'] as string);

    await delay(5 * 1000);
    this.sharedData['deactivatedDidDocument'] = await resolveDID(did, "application/did+json", { verifier: signer });

    expect(this.sharedData['deactivatedDidDocument']).not.toBeNull();
    expect(this.sharedData['deactivatedDidDocument'].verificationMethod).toHaveLength(0);
});

Given('an non existing DID with its non existing DID Document', async function (this: DIDWorld) {
    this.sharedData['did'] = "did:hedera:local-node:icSQUreA6LXir7aJq5wnnwQmjcmyGffwNfmEM48Ec4Z_0.0.0000";
});
