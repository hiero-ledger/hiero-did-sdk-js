...
export async function startContainers(configPath: string): Promise<void> {
  ...
  const { consensusNode, postgres } = await startConsensusNode(
    configPath,
    network,
  );
...
async function startConsensusNode(
  ...
  consensusNode: StartedTestContainer;
  postgres: StartedTestContainer;
}> {
  const consensusNode = await new GenericContainer(
    'gcr.io/hedera-registry/consensus-node:0.54.0-alpha.5',
  )
    ...
    .withBindMounts([
      {
        source: `${configPath}/network_logs/accountBalances`,
        target: '/opt/hgcapp/accountBalances',
      },
      {
        source: `${configPath}/network_logs/recordStreams`,
        target: '/opt/hgcapp/recordStreams',
      },
    ])
    .withCopyFilesToContainer([
      {
        source: `${configPath}/settings/network-node/entrypoint.sh`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/myentrypoint.sh',
      },
      {
        source: `${configPath}/settings/network-node/config.txt`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/config.txt',
      },
      {
        source: `${configPath}/settings/network-node/settings.txt`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/settings.txt',
      },
      {
        source: `${configPath}/settings/network-node/log4j2.xml`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/log4j2.xml',
      },
      {
        source: `${configPath}/settings/network-node/hedera.crt`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/hedera.crt',
      },
      {
        source: `${configPath}/settings/network-node/hedera.key`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/hedera.key',
      },
    ])
    .withCopyDirectoriesToContainer([
      {
        source: `${configPath}/settings/network-node/data/config`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/data/config',
      },
      {
        source: `${configPath}/settings/network-node/data/keys`,
        target: '/opt/hgcapp/services-hedera/HapiApp2.0/data/keys',
      },
      {
        source: `${configPath}/settings/record-parser`,
        target: '/opt/hgcapp/recordParser',
      },
    ])
    .withEnvironment({
      JAVA_HEAP_MIN: '256m',
      JAVA_HEAP_MAX: '2g',
      JAVA_OPTS:
        '-XX:+UnlockExperimentalVMOptions -XX:+UseZGC -Xlog:gc*:gc.log',
    })
    .withWaitStrategy(
      Wait.forLogMessage(
        'TLS gRPC server listening on port',
  ...
  const postgres = await new GenericContainer('ghcr.io/mhga24/postgres:latest')
    ...
    .withCopyFilesToContainer([
      {
        source: `${configPath}/settings/mirror-node/init.sh`,
...
async function startMirrorNodeGrpc(
  configPath: string,
  network: StartedNetwork,
  db: StartedTestContainer,
): Promise<StartedTestContainer> {
  return await new GenericContainer(
    'gcr.io/mirrornode/hedera-mirror-grpc:0.114.1',
  )
    .withPullPolicy(PullPolicy.alwaysPull())
    .withDefaultLogDriver()
    .withNetwork(network)
    .withExposedPorts({ container: 5600, host: 5600 })
    .withUser(currentUser)
    .withName('mirror-node-grpc-single')
    .withNetworkAliases('mirror-node-grpc-single')
    .withCopyFilesToContainer([
      {
        source: `${configPath}/settings/mirror-node/application.yml`,
...
async function startMirrorNodeRest(
  configPath: string,
  network: StartedNetwork,
  db: StartedTestContainer,
): Promise<StartedTestContainer> {
  return await new GenericContainer(
    'gcr.io/mirrornode/hedera-mirror-rest:0.114.1',
  )
    .withPullPolicy(PullPolicy.alwaysPull())
    .withDefaultLogDriver()
    .withNetwork(network)
    .withExposedPorts({ container: 5551, host: 5551 })
    .withUser(currentUser)
...
async function startMirrorNodeImporter(
  configPath: string,
  network: StartedNetwork,
  db: StartedTestContainer,
): Promise<StartedTestContainer> {
  return await new GenericContainer(
    'gcr.io/mirrornode/hedera-mirror-importer:0.114.1',
  )
    .withPullPolicy(PullPolicy.alwaysPull())
    .withNetwork(network)
    .withUser(currentUser)
    ...
    .withBindMounts([
      {
        source: `${configPath}/network_logs/accountBalances/balance0.0.3`,
        target: '/node/streams/accountBalances/balance0.0.3',
      },
      {
        source: `${configPath}/network_logs/recordStreams/record0.0.3`,
        target: '/node/streams/recordstreams/record0.0.3',
      },
    ])
    .withCopyFilesToContainer([
      {
      ...
      {
        source: `${configPath}/settings/mirror-node/addressBook.bin`,
        target:
          '/usr/etc/hedera-mirror-importer/local-dev-1-node.addressbook.f102.json.bin',
      },
    ])
    .withEnvironment({
      HEDERA_MIRROR_IMPORTER_DB_HOST: db.getIpAddress('bridge'),
      SPRING_CONFIG_ADDITIONAL_LOCATION:
        'file:/usr/etc/hedera-mirror-importer/',
    })
    .withWaitStrategy(Wait.forLogMessage('Started ImporterApplication'))
    .start();
}

async function startMirrorNodeMonitor(
  configPath: string,
  network: StartedNetwork,
): Promise<StartedTestContainer> {
  return await new GenericContainer(
    'gcr.io/mirrornode/hedera-mirror-monitor:0.114.1',
  )
    .withPullPolicy(PullPolicy.alwaysPull())
    .withNetwork(network)
    .withUser(currentUser)
