...
Given(
  'a private key in DER format {string}',
  function (this: DIDWorld, privateKey: string) {
    this.sharedData['privateKey'] = privateKey;
  },
);

When(
  'the `createDID` is called with the SDK Client',
...
When(
  'the `createDID` is called with the private key and the SDK Client',
  async function (this: DIDWorld) {
...
Given(
  'an instance of PrivateKey class initiated using given private key {string}',
  function (this: DIDWorld, pk: string) {
    this.sharedData['privateKey'] = PrivateKey.fromStringDer(pk);
  },
);

Then(
  'the function should return a newly created DID and its DID Document',
...
Then(
  'the function should return a newly created DID, its DID Document and Private Key',
  function (this: DIDWorld) {
    expect(this.sharedData['did']).not.toBeNull();
    expect(this.sharedData['didDocument']).not.toBeNull();
...
Then(
  'the DID should conform to {string}',
  function (this: DIDWorld, schemaFile: string) {
...
Then(
  'the DID Document should conform to {string}',
  function (this: DIDWorld, schemaFile: string) {
    const schemaPath = path.join(
      __dirname,
      '../../../../support/data',
